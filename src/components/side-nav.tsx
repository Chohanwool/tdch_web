...
import { findMatchedNavigationItem } from "@/lib/navigation-utils";
...
export default function SideNav({ groupKey, groupLabel }: SideNavProps) {
  const pathname = usePathname() ?? "";
  const { navMenuGroups } = useNavigation();

  const menuGroup = navMenuGroups.find((group) =>
    groupKey ? group.key === groupKey : group.label === groupLabel
  );
  const items = menuGroup?.items.filter((item) => !item.hiddenInLnb) || [];
  const activeItem = findMatchedNavigationItem(pathname, menuGroup);
  const displayLabel = menuGroup?.label ?? groupLabel ?? "";

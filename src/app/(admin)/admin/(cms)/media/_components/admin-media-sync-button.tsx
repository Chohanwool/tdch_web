...
import type { AdminMediaSyncActionState } from "../actions";

interface AdminMediaSyncButtonProps {
  action: (
...
export default function AdminMediaSyncButton({ action }: AdminMediaSyncButtonProps) {
  const [state, formAction, isPending] = useActionState<AdminMediaSyncActionState, FormData>(action, {});
  const [toast, setToast] = useState<AdminMediaSyncActionState | null>(null);
    ...
    <>
      ...
      <form action={formAction}>
        <button
          type="submit"
          disabled={isPending}
          className="inline-flex h-9 items-center gap-2 rounded-lg border border-[#bfd0ea] bg-[#eff6ff] px-4 text-[13px] font-semibold text-[#2d5da8] transition hover:bg-[#e4efff] disabled:cursor-not-allowed disabled:opacity-60"
        >

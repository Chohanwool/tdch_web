...
export default function Home() {
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-16 px-6 pb-20 pt-12">
      <section className="fade-up rounded-3xl border border-cedar/10 bg-ivory/80 p-8 shadow-soft sm:p-12">
        ...
        <p className="mt-5 max-w-2xl text-sm text-ink/70 sm:text-base">
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          <Link
            href="/newcomer"
            className="rounded-full bg-clay px-5 py-3 text-sm font-semibold text-ivory transition hover:bg-cedar"
          >
            새가족 등록하기
          </Link>
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="rounded-full border border-cedar/25 px-5 py-3 text-sm font-semibold text-cedar transition hover:border-clay hover:text-clay"
          >
            예배 영상 보기
          </a>
        </div>
      ...
      <section className="space-y-6">
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          {serviceTimes.map((item) => (
            <article key={item.title} className="rounded-2xl border border-cedar/10 bg-white/70 p-5">
      ...
      <section className="space-y-6">
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          {sermons.map((sermon) => (
            <article key={sermon.title} className="rounded-2xl border border-cedar/10 bg-white/80 p-5">
      ...
      <section className="space-y-6">
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          {newsPosts.map((post) => (
            <article key={post.title} className="rounded-2xl border border-cedar/10 bg-white/75 p-5">
